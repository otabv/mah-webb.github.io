...
Som en avslutande övning och för att ni ska få chans att skapa HTML-kod helt själva med eget innehåll kan ni nu tömma er `<body>` på alla kodexempel (eller spara undan dessa i ett annat HTML-dokument). Vidare ska ni nu fylla `<body>` med följande delar, innehåll i form av text och bilder är valfritt men bör vara realistiska, ni kan exempelvis kopiera text från webben.
