...
<!-- Följande mall kan användas för att testa vårat exempel. -->
<p>
    When a doctor does go wrong, <i class="important">he is</i> the first of criminals.
</p>
{% endhighlight %}
...
/* Det första närliggande paragrafen kommer få röd färg. */
p + p {
    color: Red;
}
...
En kombination (kedja) av selektorer separeras med ett mellanrum `.class p #id { ... }`. För att explicit bestämma att alla element av en typ med en klass ska få en stilmall skriver man `p.important { ... }`.
{: .info}
...
Följande attribut måste användas på ett valfritt HTML dokument:
