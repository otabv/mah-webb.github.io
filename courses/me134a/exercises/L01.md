...
Länkar är sen tidigare förmodligen inget nytt, inom HTML skapas dessa med elementet `<a></a>` (eng. anchor). Detta elementet kräver huvudsakligen två saker: 1) det innehåll som skall länkas, t.ex en ord eller en bild. 2) den URL som användaren ska skickas till. Nummer ett av dessa placeras inom start- och sluttaggarna, t.ex `<a>Klicka mig!</a>`, och nummer två skapas genom attributet `href`.
